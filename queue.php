...
<script language="javascript">
	/*
	SLIMCD CREDIT CARD FORM DEMO USING GetTemporaryToken 
	*/
	...
	$(document).ready(function() {
		...
		$("#setBilling").click(function(){
			...
			//bind cardNumLoad function to cardNum & cardCVV input on value change
		$("input#cardNum,input#cardCVV").change(function(){
				//if has value, clear error class
			if($(this).val()!=""){ 
				$(this).removeClass('err');
				$("[for='"+$(this).attr('id')+"']").removeClass('err');
			}
				//if cardNum and cardCVV already loaded then set data as no longer loaded, clear other value and add error class
			if($('#cardNum').data('load') == 'yes' || $('#cardCVV').data('load') == 'yes'){
				$(this).data('load','no');
				$("input#cardNum,input#cardCVV").not($(this)).data('load','no').val('').addClass('err');
				$('label[for=cardNum],label[for=cardCVV]').not('label[for='+$(this).attr('id')+']').addClass('err');
			}else { //otherwise call cardNumLoad and function
				cardNumLoad();
			}
		});
	...
	function setBilling(){
		...
		//use GetTemporaryToken to send CVV2 & cardNum to SlimCD and return token
	function cardNumLoad(){
			//check that cardCVV have a value, cardNum is validated and cardNum not already loaded
		if($('#cardCVV').val() != '' && $("#cardNum").hasClass('valid') && $('#cardNum').data('load') != 'yes'){
			SlimCD.Transact.GetTemporaryToken({
				...
				function(reply){
					...
					if (reply.response == 'Success') {
						...
						$('label[for=cardCVV],label[for=cardNum]').removeClass('err');
